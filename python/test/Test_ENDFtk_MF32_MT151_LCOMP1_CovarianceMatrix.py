...
class Test_ENDFtk_MF32_MT151_LCOMP1_CovarianceMatrix( unittest.TestCase ) :
    """Unit test for the CovarianceMatrix class."""

    chunk = ( ' 0.000000+0 0.000000+0          0          0         36          8102532151     \n'
    ...
    zero_size = ( ' 0.000000+0 0.000000+0          0          0          0          8102532151     \n' )

    no_number_parameters = ( ' 0.000000+0 0.000000+0          0          0         36          0102532151     \n'
                             ' 5.000000+0 6.000000+0 7.000000+0 8.000000+0 9.000000+0 1.000000+1102532151     \n'
                             ' 1.100000+1 1.200000+1 1.300000+1 1.400000+1 1.500000+1 1.600000+1102532151     \n'
                             ' 1.700000+1 1.800000+1 1.900000+1 2.000000+1 2.100000+1 2.200000+1102532151     \n'
                             ' 2.300000+1 2.400000+1 2.500000+1 2.600000+1 2.700000+1 2.800000+1102532151     \n'
    ...
    def test_component( self ) :
        ...
        chunk = CovarianceMatrix(  values = [ 5.,  6.,  7.,  8.,  9., 10., 11., 12.,
        ...
        chunk = CovarianceMatrix.from_string( self.chunk, 1025, 32, 151 )

        verify_chunk( self, chunk )

        # the data is copied
        copy = CovarianceMatrix( chunk )
    ...
    def test_failures( self ) :
        ...
        with self.assertRaises( Exception ) :
            ...
            chunk = CovarianceMatrix( values = [] )

        # invalid list size
        with self.assertRaises( Exception ) :

            chunk = CovarianceMatrix.from_string( self.invalid_size,
                                                  1025, 32, 151 )

        # no data
        with self.assertRaises( Exception ) :

            chunk = CovarianceMatrix.from_string( self.zero_size,
                                                  1025, 32, 151 )

        # no number resonances
        with self.assertRaises( Exception ) :

            chunk = CovarianceMatrix.from_string( self.no_number_parameters,
                                                  1025, 32, 151 )
