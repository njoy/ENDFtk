...
class Test_ENDFtk_MF8_MT457_Section( unittest.TestCase ) :
    ...
    def test_section( self ) :
        ...
        def verify_chunk( self, chunk ) :
            ...
            self.assertAlmostEqual( 3, continuous.number_points )
            self.assertAlmostEqual( 1, continuous.NR )
            self.assertAlmostEqual( 1, continuous.number_interpolation_regions )
            self.assertAlmostEqual( 1, len( continuous.INT ) )
            self.assertAlmostEqual( 1, len( continuous.NBT ) )
            self.assertAlmostEqual( 1, continuous.INT[0] )
            self.assertAlmostEqual( 3, continuous.NBT[0] )
        ...
        chunk = tape.materials.front().file( 8 ).section( 457 ).parse()
