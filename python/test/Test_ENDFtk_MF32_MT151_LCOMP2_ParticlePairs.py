...
class Test_ENDFtk_MF32_MT151_LCOMP2_ParticlePairs( unittest.TestCase ) :
    ...
    invalid_size = ( ' 0.000000+0 0.000000+0          2          1         23          4262532151     \n'
    ...
    zero_size = ( ' 0.000000+0 0.000000+0          2          1          0          4262532151     \n' )

    zero_npp = ( ' 0.000000+0 0.000000+0          0          1         24          0262532151     \n'
                 ' 0.000000+0 5.446635+1 2.000000+0 2.600000+1 1.000000+0 4.000000+0262532151     \n'
                 ' 6.000000+0 8.000000+0 1.000000+1 1.020000+2 1.200000+1 1.400000+1262532151     \n'
                 ' 1.000000+0 5.347624+1 3.000000+0 2.700000+1 5.000000-1 5.000000+0262532151     \n'
                 ' 7.000000+0 9.000000+0 1.100000+1 2.000000+0 1.300000+1 1.500000+1262532151     \n' )
    ...
    def test_component( self ) :
        ...
        chunk = ParticlePairs( njsx = 1, ma = [ 0., 1. ],
                               mb = [ 5.446635e+1, 5.347624e+1 ],
                               za = [ 2., 3. ],  zb = [ 26., 27. ],
                               ia = [ 1., 0.5 ], ib = [ 4., 5. ],
                               q = [ 6., 7. ], pnt = [ 8, 9 ],
                               shf = [ 10, 11 ], mt = [ 102, 2 ],
        ...
        chunk = ParticlePairs.from_string( self.chunk, 2625, 32, 151 )

        verify_chunk( self, chunk )

        # the data is copied
        copy = ParticlePairs( chunk )
    ...
    def test_failures( self ) :
        ...
        with self.assertRaises( Exception ) :
            ...
            chunk = ParticlePairs.from_string( self.invalid_size, 2625, 32, 151 )

        # no data
        with self.assertRaises( Exception ) :

            chunk = ParticlePairs.from_string( self.zero_size, 2625, 32, 151 )

        # no number resonances
        with self.assertRaises( Exception ) :

            chunk = ParticlePairs.from_string( self.zero_npp, 2625, 32, 151 )
