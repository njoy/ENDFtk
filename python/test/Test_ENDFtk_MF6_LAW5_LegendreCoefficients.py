...
class Test_ENDFtk_MF6_LegendreCoefficients( unittest.TestCase ) :
    ...
    def test_component( self ) :
        ...
        def verify_chunk( self, chunk ) :
            ...
            self.assertEqual( 4, len( chunk.C ) )
            self.assertEqual( 4, len( chunk.coefficients ) )
            self.assertAlmostEqual( 1., chunk.C[0] )
            self.assertAlmostEqual( 2., chunk.C[1] )
            self.assertAlmostEqual( 3., chunk.C[2] )
            self.assertAlmostEqual( 4., chunk.C[3] )
