...
class Test_ENDFtk_MF4_MixedDistributions( unittest.TestCase ) :
    ...
    def test_component( self ) :
        ...
        def verify_chunk( self, chunk ) :
            ...
            self.assertEqual( 2, chunk.legendre.number_incident_energies )
            self.assertEqual( 1, chunk.legendre.NR )
            self.assertEqual( 1, chunk.legendre.number_interpolation_regions )
            self.assertEqual( 1, len( chunk.legendre.INT ) )
            self.assertEqual( 1, len( chunk.legendre.NBT ) )
            self.assertEqual( 1, chunk.legendre.INT[0] )
            self.assertEqual( 2, chunk.legendre.NBT[0] )
            ...
            self.assertEqual( 2, chunk.tabulated.number_incident_energies )
            self.assertEqual( 1, chunk.tabulated.NR )
            self.assertEqual( 1, chunk.tabulated.number_interpolation_regions )
            self.assertEqual( 1, len( chunk.tabulated.INT ) )
            self.assertEqual( 1, len( chunk.tabulated.NBT ) )
            self.assertEqual( 5, chunk.tabulated.INT[0] )
            self.assertEqual( 2, chunk.tabulated.NBT[0] )
