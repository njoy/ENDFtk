...
# local imports
from ENDFtk.MF32.MT151 import GeneralRMatrixLimited
...
class Test_ENDFtk_MF32_MT151_GeneralRMatrixLimited( unittest.TestCase ) :
    """Unit test for the GeneralRMatrixLimited class."""

    chunk = ( ' 0.000000+0 0.000000+0          0          1          0          0102532151     \n'
    ...
    def test_component( self ) :
        ...
        def verify_chunk( self, chunk ) :
            ...
            self.assertEqual( 2, block.NJSX )
            self.assertEqual( 2, block.number_spin_groups )
            self.assertEqual( 2, len( block.resonance_parameters ) )

            parameters1 = block.resonance_parameters[0]
        ...
        chunk = GeneralRMatrixLimited.from_string( self.chunk, 1025, 32, 151 )

        verify_chunk( self, chunk )

        # the data is copied
        copy = GeneralRMatrixLimited( chunk )
