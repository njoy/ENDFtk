...
class Test_ENDFtk_MF12_Multiplicities( unittest.TestCase ) :
    ...
    def test_component( self ) :
        ...
        def verify_chunk_partials( self, chunk ) :
            ...
            self.assertEqual( 2, total.number_points )
            self.assertEqual( 1, total.NR )
            self.assertEqual( 1, total.number_interpolation_regions )
            self.assertEqual( 1, len( total.INT ) )
            self.assertEqual( 1, len( total.NBT ) )
            self.assertEqual( 2, total.INT[0] )
            self.assertEqual( 2, total.NBT[0] )
