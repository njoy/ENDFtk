...
SCENARIO( "Testing Material" ) {
  ...
  GIVEN( "a string representation of a Material containing only MF1 MT451" ) {
    ...
    WHEN( "a Material is constructed from a syntaxTree" ) {

      auto begin = matstring.begin();
      auto start = matstring.begin();
      auto end = matstring.end();
      long lineNumber = 0;
...
void verifyMaterial( const Material& chunk ) {
  ...
  CHECK( 131 == chunk.MAT() );
  CHECK( 131 == chunk.materialNumber() );
  ...
  CHECK( chunk.hasMF( 4 ) );
  CHECK( chunk.hasMF( 5 ) );
  CHECK( not chunk.hasMF( 6 ) );
  CHECK( chunk.hasFile( 1 ) );
  CHECK( chunk.hasFile( 2 ) );
  CHECK( chunk.hasFile( 3 ) );
  CHECK( chunk.hasFile( 4 ) );
