...
void verifyMaterial( const Material& chunk ) {

  CHECK( 125 == chunk.MAT() );
  CHECK( 125 == chunk.materialNumber() );

  CHECK( chunk.hasMF( 1 ) );
  CHECK( not chunk.hasMF( 2 ) );
  CHECK( chunk.hasFile( 1 ) );
  CHECK( not chunk.hasFile( 2 ) );

  CHECK_NOTHROW( chunk.MF( 1_c ) );
  CHECK_NOTHROW( chunk.file( 1_c ) );
}
