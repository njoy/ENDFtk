...
SCENARIO( "Testing Resolved Resonance ReichMoore" ){
  GIVEN( "valid ENDF parameters with TAB1" ){
    ...
    WHEN( "NRO != 0" ){
      ...
      ResonanceParameters::Resolved::BreitWigner rm(
          std::move(base), std::move(tab1), 
          begin, end, lineNumber, MAT, MF, MT);
      THEN( "the parameters can be verified" ){
        REQUIRE( 1E-5 == rm.lowerEnergyLimit );
        REQUIRE( 3.2 == rm.upperEnergyLimit );
        REQUIRE( 1 == rm.NRO );
        REQUIRE( 0 == rm.NAPS );

        {
        REQUIRE( 3.0 == rm.cont.C1() );
        REQUIRE( 0.0 == Approx( rm.cont.C2() ) );
        REQUIRE( 0 == rm.cont.L1() );
        REQUIRE( 0 == rm.cont.L2() );
        REQUIRE( 1 == rm.cont.N1() );
        REQUIRE( 0 == rm.cont.N2() );
        }

        {
        REQUIRE( 166.492 == Approx( rm.list.C1() ) );
        REQUIRE( 0.0 == rm.list.C2() );
        REQUIRE( 0 == rm.list.L1() );
        REQUIRE( 0 == rm.list.L2() );
        REQUIRE( 24 == rm.list.NPL() );
        REQUIRE( 4 == rm.list.N2() );
        }

        {
          REQUIRE( 0.0 == (*rm.tab1).C1() );
          REQUIRE( 0.0 == (*rm.tab1).C2() );
          REQUIRE( 0 == (*rm.tab1).L1() );
