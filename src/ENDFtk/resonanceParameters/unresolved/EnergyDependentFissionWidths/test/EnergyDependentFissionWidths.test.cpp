...
using EnergyDependentFissionWidths = resonanceParameters::unresolved::EnergyDependentFissionWidths;
...
SCENARIO( "EnergyDependentFissionWidths" ) {
  ...
  GIVEN( "valid data for a EnergyDependentFissionWidths" ) {
    ...
    WHEN( "the data is given explicitly" ) {
      ...
      EnergyDependentFissionWidths chunk( spi, ap, lssf,
                                          std::move( energies ),
                                          std::move( lvalues ) );

      THEN( "a EnergyDependentFissionWidths can be constructed and members can "
            "be tested" ) {
    ...
    WHEN( "the data is read from a string/stream" ) {
      ...
      EnergyDependentFissionWidths chunk( begin, end, lineNumber,
                                          9440, 2, 151 );
...
void verifyChunk( const EnergyDependentFissionWidths& chunk ) {
