...
public:
   ...
   */
  decltype(auto) T() const { return TabulationRecord::x(); }

  /**
   *  @brief Return the temperature values
   */
  decltype(auto) temperatures() const { return this->T(); }

  /**
   *  @brief Return the Debye-Waller integral divided by the atomic mass (eV^-1)
   */
  decltype(auto) W() const { return TabulationRecord::y(); }

  /**
   *  @brief Return the Debye-Waller integral divided by the atomic mass (eV^-1)
   */
  decltype(auto) debyeWallerValues() const { return this->W(); }
