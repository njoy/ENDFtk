 ...
 */
Type( double zaid, double awr, long iinel, long iabso, long nbin,
      ListRecord&& list ) :
  BaseWithoutMT( zaid, awr ), inelastic_comp_( iinel ),
  absorption_comp_( iabso ), nbin_( nbin ), data_( std::move( list ) ) {
...
//
//    Log::info( "Trouble while constructing section 153 of File 2" );
//    throw e;
//  }
...
Type ( const HEAD& head,
       ...
       int MAT )
  try:
    Type( head.ZA(), head.AWR(), head.L1(), head.L2(), head.N2(),
          ListRecord( begin, end, lineNumber, head.MAT(), 2, 153 ) ) {

      readSEND( begin, end, lineNumber, MAT, 2 );
  } catch( std::exception& e ){
    Log::info( "Trouble while reading section 153 of File 2 of Material {}",
               MAT );
    throw e;
  }
