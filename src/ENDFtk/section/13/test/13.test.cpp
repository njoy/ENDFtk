  ...
  GIVEN( "a string representation of a valid File 3 Section" ){
    WHEN( "NK = 1" ){
      ...
      THEN( "a section::Type<13> can be constructed and members can be tested" ){
        ...
        REQUIRE( std::nullopt == MF13.table );

        REQUIRE( 1 == MF13.subsections.size() );
        REQUIRE( std::vector< double >() == MF13.energies() );
        REQUIRE( std::vector< double >() == MF13.crossSections() );
      }
    } // WHEN
    ...
    WHEN( "creating an intermediate syntaxTree::Section" ){
      ...
      syntaxTree::Section< std::string::iterator> mf13(
          head, begin, position, end, lineNumber );
    ...
    WHEN( "NK = 2" ){
      ...
      std::string sectionString = sHead + 
          baseSection() + baseSection() + baseSection() + validSEND();
      auto begin = sectionString.begin();
      auto end = sectionString.end();
      long lineNumber = 0; 
      HeadRecord head( begin, end, lineNumber );
      ...
      THEN( "a section::Type<13> can be constructed and members can be tested" ){
        ...
        std::vector< double > energies = MF13.energies();
        std::vector< double > XS = MF13.crossSections();
    ...
    WHEN( "NK is wrong" ){
      std::string sHead = 
        " 8.016000+3 1.585751+1          0          0          2          0 82513 22\n";
      std::string sectionString = sHead + 
          baseSection() + baseSection() + validSEND();
      auto begin = sectionString.begin();
      auto end = sectionString.end();
      long lineNumber = 0; 
      HeadRecord head( begin, end, lineNumber );
