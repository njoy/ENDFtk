...
public:
  ...
  /**
   *  @brief Return the radiation type for which spectral data is given
   *
   *  This is the STYP ENDF variable, see ENDF102, section 8.4c for more
   *  information.
   */
  double radiationType() const { return this->data_.C2(); }
   ...
   */
  auto discreteNormalisationFactor() const {

    return std::tuple< double, double >( this->data_.list()[0],
                                         this->data_.list()[1] );
  }
   ...
   */
  auto continuousNormalisationFactor() const {

    return std::tuple< double, double >( this->data_.list()[4],
                                         this->data_.list()[5] );
  }
   ...
   */
  auto averageDecayEnergy() const {

    return std::tuple< double, double >( this->data_.list()[2],
                                         this->data_.list()[3] );
  }
   ...
   */
  const auto& discreteSpectra() const { return this->discrete_; } 
