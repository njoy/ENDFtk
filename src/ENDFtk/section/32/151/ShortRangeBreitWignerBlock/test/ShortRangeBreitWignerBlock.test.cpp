...
using ShortRangeBreitWignerBlock =
section::Type< 32, 151 >::ShortRangeBreitWignerBlock;

std::string chunk();
void verifyChunk( const ShortRangeBreitWignerBlock& );
std::string invalidSize();
std::string zeroSize();
std::string noNumberResonances();

SCENARIO( "ShortRangeBreitWignerBlock" ) {

  GIVEN( "valid data for a ShortRangeBreitWignerBlock" ) {
    ...
    WHEN( "the data is given explicitly" ) {
      ...
      THEN( "a ShortRangeBreitWignerBlock can be constructed and members can be "
            "tested" ) {
    ...
    WHEN( "the data is read from a string/stream" ) {
      ...
      ShortRangeBreitWignerBlock chunk( begin, end, lineNumber, 1025, 32, 151 );
  ...
  GIVEN( "invalid data" ) {
    ...
    WHEN( "a string with inconsistent NPL and NRB is used" ) {
      ...
      THEN( "an exception is thrown" ) {
        ...
        CHECK_THROWS( ShortRangeBreitWignerBlock( begin, end, lineNumber, 1025, 32, 151 ) );
      } // THEN
    } // WHEN
