...
using CompactSingleLevelBreitWigner =
section::Type< 32, 151 >::CompactSingleLevelBreitWigner;
...
std::string chunk();
void verifyChunk( const CompactSingleLevelBreitWigner& );
...
SCENARIO( "CompactCorrelationMatrix" ) {
  ...
  GIVEN( "valid data for a CompactCorrelationMatrix" ) {
    ...
    WHEN( "the data is given explicitly" ) {
      ...
      CompactSingleLevelBreitWigner chunk( spi, ap, dap,
                                          std::move( parameters ),
                                          std::move( matrix ) );

      THEN( "a CompactSingleLevelBreitWigner can be constructed and members can "
            "be tested" ) {
    ...
    WHEN( "the data is read from a string/stream" ) {
      ...
      CompactSingleLevelBreitWigner chunk( begin, end, lineNumber, 8228, 32, 151 );
...
void verifyChunk( const CompactSingleLevelBreitWigner& chunk ) {
