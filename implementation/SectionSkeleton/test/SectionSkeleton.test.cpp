...
int main( int argc, const char* argv[] ){
  LOG(INFO) << "";
  LOG(INFO) << "SectionSkeleton Tests";
  LOG(INFO) << "======================";
  int result = Catch::Session().run( argc, argv );
  LOG(INFO) << "SectionSkeleton Complete!";
  return result;
}
...
SCENARIO( "Creating a skeleton of an ENDF Section" ){
  GIVEN( "A string representation of a Section" ){
    WHEN( "a valid SEND record ends the Section" ){
      ...
      SectionSkeleton< std::string::iterator >
        sectionSkeleton( head, start, position, end, lineNumber );
      ...
      AND_THEN( "the buffer iterators are populated correctly "){
        REQUIRE( sectionString.begin() == sectionSkeleton.bufferBegin() );
        REQUIRE( end == sectionSkeleton.bufferEnd() );
      }
    ...
    WHEN( "an invalid (MT!=0) SEND record ends the Section" ){
      ...
      THEN( "an exception is thrown" ){
        ...
        REQUIRE_THROWS(
          SectionSkeleton< std::string::iterator >
          sectionSkeleton( head, start, begin, end, lineNumber ) );
      }
    } // WHEN
